...
%         
        function obj = wtbUpdate(obj, masterState)
            for i=2:length(obj.rxBuff)
                curMsg = wtbMsg.raw2wtb(obj.rxBuff{i});
                
                switch(masterState)
                    case wtbEnum.read
                        obj.compoTable(curMsg.src + 1).function = ...
                        curMsg.data(1) + 512*curMsg.data(2);
                    case wtbEnum.driving
                        obj.compoTable(curMsg.src + 1).value = ...
                            curMsg.data(1) + 32*curMsg.data(2);
                end
